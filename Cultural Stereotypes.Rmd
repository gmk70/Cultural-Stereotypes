---
title: "Cultural Stereotypes Paper"
author: "GMK"
date: "2023-04-22"
output:
...
stereotypes_combined_short <- read_csv("data/Stereotypes_Combined_Files_short.csv") %>%
  summarise(surveyyear, age, ethnicit, gender, targend, adjectiv) 
```
```{r}
read_excel("data/Stereotypes_Combined_Files_short.xlsx") %>%
  # group_by(surveyyear) %>% 
  mutate(observed = 1) %>% 
  pivot_wider(particip:targend,
              names_from = "adjectiv", 
              values_from = "observed") %>% 
  # mutate(across(ambitious:honest, ~as.integer(.))) %>% 
  mutate(ambitious = map(ambitious, ~as.numeric(.)))
  View()
```

```{r}
stereotypes <- read_excel("data/Cultural_Stereotypes_2022.xlsx") %>%
  count(Race)
...
ggplot(data = stereotypes,
  ...
  geom_point() +
    ...
    scale_y_continuous(limits = c(0, 8),
                     breaks = seq(0, 8, by = 1)) +
  geom_dumbbell(colour_x = "gray",
  colour =- "gray",
  colour)
  labs(title = "Multicultural Perspectives of AA Men's Smart Stereotypes by Race",
       x = "Smart Stereotype of Asian Americans",
       y = "Multicultural Perspective",
       fill = "") +
  theme_clean() 
...
```{r}
ggplot(diamonds, aes(x = "MCB_Blind", y = "Race")) +
...
```{r}
read_csv("data/Stereotypes_Combined_Files_short II.csv") %>%
  clean_names() %>% 
